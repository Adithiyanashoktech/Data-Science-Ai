...
- **AI Insights Fallback Logic Correction**: Resolved a critical backend `NameError: name 'column' is not defined` and a `TypeError: unsupported format string passed to NoneType` inside the local heuristic engine fallback. This occurred when any dataset query fell through to the local NLG templates without an active Gemini API key. Added correct parameter bindings for the active `column` and wrapped fallback templates in `safe_fmt` calls.
